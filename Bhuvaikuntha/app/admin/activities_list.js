...
const activityList= () => {
  ...
  setRefreshing(false);
    ...
    try {
      const response = await fetch("https://radhavallabha-backend-599500600776.asia-south1.run.app/api/get/activity");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://radhavallabha-backend-599500600776.asia-south1.run.app/api/delete/activity/${id}`, {
        method: 'DELETE',
      });
...
const renderPost = ({ item }) => (
  <TouchableOpacity
   onPress={() =>
        router.push({
          pathname: './activitiesParticipantList',
          params: {
            id: item.id,
  ...
  >
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        <Text style={styles.place}>{item.heading}</Text>
  ...
  return (
    <>
      ...
      No Activity found
        ...
        <TouchableOpacity
          style={styles.fab}
          onPress={() => router.push('./our_activities')}
        >
